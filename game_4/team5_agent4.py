    ...
    def get_move_weight(self, player_id, move):
        ...
        for target_direction in target_possible_directions:
            ...
            if 0 <= new_x < self.len_x and 0 <= new_y < self.len_y \
                ...
                and self.map_stat[new_x][new_y] != (player_id + 2) % 4 and self.sheep_stat[new_x][new_y] > 1:
                weight += 3
    ...
    def backpropagate(self, node, result):
        ...
        while node is not None:
            ...
            for i in range(len(node.state.player_ids)):
                ...
                j = (i + 2) % self.root.state.num_players
                node.wins[i] += magic_func(scores[j], k=(1/7), a=32, c=1) * (4 - ranking.index(node.state.player_ids[j]))
            node = node.parent
