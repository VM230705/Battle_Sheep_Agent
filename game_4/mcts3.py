...
class MCTSNode:
    ...
    def simulate(self):
        ...
        # 与自己组合的玩家ID
        partner_player = (self.playerID + 2) % 4 + 1
        ...
        winning_group = []
        for i in range(1, 3):  # Iterate over player IDs in steps of 2 to form groups
            group_score = player_scores[i] + player_scores[i+2]
            winning_group.append((i, group_score))

        winner = max(winning_group, key=lambda x: x[1])[0]  # Find the player ID with the highest group score
        return winner
