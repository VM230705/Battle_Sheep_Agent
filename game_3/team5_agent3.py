...
class GameState:
    ...
    def is_terminal(self):
        player_gameover_count = 0
        for player_id in self.player_ids:
            count = 0
            for i in range(self.len_x):
                for j in range(self.len_y):
                    if self.map_stat[i][j] == player_id:
                        count += 1
            if count == 16 or not self.get_possible_moves(player_id):
                player_gameover_count += 1
        if player_gameover_count == 4:
            return True
        return False
    ...
    def backpropagate(self, node, result):
        ...
        while node is not None:
            ...
            for i in range(len(node.state.player_ids)):
                ...
                node.wins[i] += magic_func(scores[i], k=(1/7), a=32, c=1) * (4 - ranking.index(node.state.player_ids[i]))
            node = node.parent
...
def mcts(playerID, mapStat, sheepStat):
    ...
    for i in range(len(mapStat)):
        for j in range(len(mapStat[0])):
            if mapStat[i][j] in [1, 2, 3, 4] and mapStat[i][j] != playerID:
                sheepStat[i][j] = 16
